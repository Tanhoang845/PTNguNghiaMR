...
Type* compileLValue(void) {
  // TODO: parse a lvalue (a variable, an array element, a parameter, the current function identifier) ->OK
  Object* var;
  Type* varType;
  ...
  switch (var->kind){
    case OBJ_VARIABLE:
      if(var->varAttrs->type->typeClass == TP_ARRAY)
        compileIndexes(var->varAttrs->type);
      else 
        varType = var->typeAttrs->actualType; 
      break;
    case OBJ_PARAMETER:
...
void compileAssignSt(void) {
  // TODO: parse the assignment and check type consistency -> OK
  Type *varType;
  Type *expType;
...
void compileForSt(void) {
  // TODO: Check type consistency of FOR's variable -> OK 
  Type *varType;
  Type *type;
  eat(KW_FOR);
  eat(TK_IDENT);
...
void compileArgument(Object* param) {
  ...
  //       If the corresponding parameter is a reference, the argument must be a lvalue -> OK
  Type * type;
...
void compileArguments(ObjectNode* paramList) {
  //TODO: parse a list of arguments, check the consistency of the arguments and the given parameters -> OK
...
void compileCondition(void) {
  // TODO: check the type consistency of LHS and RSH, check the basic type -> OK
  Type *type1;
  Type *type2;
