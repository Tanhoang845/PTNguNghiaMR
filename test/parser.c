...
//B2------------------------------------------------------------------------------------------
void compileSWITCH(void){
  Type *type;
  ConstantValue *constant;
  assert("Parsing an Switch statement ....");
  eat(KW_SWITCH);
  ...
  while(lookAhead->tokenType == KW_CASE || lookAhead->tokenType == KW_BREAK){
    eat(KW_CASE);
    constant = compileConstant();
    if(constant->type != TP_INT)
  ...
  eat(KW_END);
  assert("Parsing an Switch parsed ....");
}
